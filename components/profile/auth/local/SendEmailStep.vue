<template>
    <div>
        <div class="custom-form-message">На Вашу почту будет отправлено письмо с кодом для восстановления пароля</div>
        ...
        <b-form autocomplete="off" novalidate @submit.prevent="sendEmail">
            <b-form-group>
                <b-input
                    ...
                    :state="email.state"
                ></b-input>
                <b-form-invalid-feedback>{{ email.error }}</b-form-invalid-feedback>
            </b-form-group>
...
export default {
    ...
    watch: {
        ...
        serverErrors(value) {
            ...
            this.email.error = value.email && value.email[0] ? value.email[0] : this.email.error;
        },
    },
