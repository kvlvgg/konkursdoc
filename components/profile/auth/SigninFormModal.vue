<template>
  <div>
    <div class="font-weight-bold custom-modal-title">Вход в аккаунт</div>
    ...
    <div class="custom-modal-question font-weight-normal">
      <span class="align-middle">Нет аккаунта?</span>
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      if (response === 'Email is not confirmed') {
      ...
      if (response.status != 'success') {
        this.setErrorState(
          typeof response === 'object'
            ? response
            : { email: [response], password: [response] }
        );
        return;
      }
