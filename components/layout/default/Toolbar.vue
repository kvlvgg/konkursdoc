<template>
    <header :class="$style['toolbar']">
        <div :class="$style['icon-menu-expander']" @click="$emit('expand-nav-bar')" />
        ...
        <div v-if="notificationCount" :class="$style['count']">
          {{ notificationCount }}
            ...
            <nuxt-link exact to="/profile" :class="$style['item']">
                <img src="~assets/svg/toolbar/person.svg" alt="person" :class="$style['avatar']" />

                <div :class="$style['name']">{{ $user.firstname }}</div>
            </nuxt-link>

            <div :class="$style['separator']" />

            <div :class="$style['item']">
                <div id="toolbar-logout" :class="$style['logout-btn']" @click="$store.dispatch('auth/logout')" />

                <b-popover
                    :custom-class="$style['popover-target']"
                    target="toolbar-logout"
                    triggers="hover"
...
export default {
    ...
    created() {
        this.$store.dispatch('common/loadExchangeRate');
    },
...
@import '~/assets/scss/module/icons/logout.scss';
