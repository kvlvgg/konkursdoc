<template>
    <div class="font-weight-light">
        ...
        <div>
            <b-button block variant="primary" class="custom-modal-button" @click="save"> Сохранить </b-button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
         ...
         */
        goToYandexMarket() {
            window.open(`https://market.yandex.ru/search?text=${this.vendor}%20${this.partNumber}`, '_blank');
        },
        /**
         * Маска для Input
         * @param value
        ...
        createMask(value) {
            ...
            const integer =
                indexOfDecimalSymbol === -1 ? validatedValue : validatedValue.slice(0, indexOfDecimalSymbol);

            const integerLength = integer.length <= integerLimit ? integer.length : integerLimit;
            ...
            if (indexOfDecimalSymbol !== -1) mask.splice(mask.length, 0, ...[/[.]/, /\d/, /\d/]);
