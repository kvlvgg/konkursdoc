    ...
    <div :class="$style['document']">
        <div :class="[$style['info'], 'font-weight-light']">
            <Panel :bodyPadding="10">
                <div :class="[$style['header'], 'font-weight-bold']">Мой проект 001</div>

                <span>{{ currenctDocumentType }}</span>
                001 &nbsp;&nbsp;<span :class="$style['number']">#0000001</span>
                <br />

                <span v-if="currentDocument.items">
                    {{ currentDocument.items.length }}
                    {{ declOfNum(currentDocument.items.length, ['позиция', 'позиции', 'позиций']) }}
                </span>
                <b-button variant="link" @click="$emit('go-to-specification-tab')"> Показать список </b-button>
            </Panel>
        ...
        <div :class="$style['editor']">
            ...
            <Calculation
                ...
                ref="table"
                :items="items"
                :currency="currentDocument && currentDocument.currency"
            />
        </div>
        ...
        <div class="invisible-scroll" :class="$style['actions']">
            <div class="font-weight-light">
                <Panel header="Работа с документами">
                    <b-button block variant="primary" size="sm" :class="$style['button']" @click="Print">
                        Распечатать
                    </b-button>
                    ...
                    <b-button
                        ...
                        size="sm"
                        :class="$style['button']"
                        @click="currenctDocumentType === 'Заявка' ? Export2Doc() : Export2Excel()"
                    >
                        Скачать
                    </b-button>
                </Panel>
                ...
                <Panel header="Другие документы">
                    <DocumentItem
                        ...
                        @print="Print"
                        @download="currenctDocumentType === 'Заявка' ? Export2Doc() : Export2Excel()"
                    />
...
import declOfNum from '~/mixins/formatters/declOfNum.js';
...
export default {
    ...
    methods: {
        ...
        Print() {
            this.currenctDocumentType === 'Заявка' ? this.$refs.editor.Print() : this.$refs.table.Print();
        },
    },
};
</script>
