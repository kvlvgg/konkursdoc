<template>
    <div :class="[$style['doc-entry'], isSelected ? $style['selected'] : '']" @click="$emit('click')">
        <!-- v-b-tooltip.hover.top="doc['Документ']" -->
        <div :class="$style['image']">
            <span v-if="document.extension === '.docx'" :class="$style['icon-docx']" />

            <span v-if="document.extension === '.xslx'" :class="$style['icon-xlsx']" />
        </div>
        ...
        <div :class="$style['image']" @click="isSelected ? $emit('print') : ''">
            <span :class="$style['icon-printer']" />
        </div>

        <div :class="$style['image']" @click="isSelected ? $emit('download') : ''">
