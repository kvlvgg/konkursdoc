...
<p class="documentTemplate" align="justify" style="text-indent: 0.49in; margin-top: 0.2in;">
...
export default function generateTemplate(items) {
    ...
    items.forEach(
        x =>
            (tbodyInner += `<tr>
        <td>${x.RowNum}</td>
        <td>${x.SubType}<br/>${x.Partnumber}</td>
        <td>${x.Vendor}</td>
        <td>${x.Country}</td>
        <td style="text-align: left;">
            ...
            ${
                x.ResourseMeasure
                    ? x.ResourseMeasure.match(/стр/)
                        ? 'Ресурс ' +
                          String(x.Resourse) +
                          ' страниц формата А4 при ' +
                          String(x.Percent) +
                          ' % заполнении'
                        : 'Ресурс ' + String(x.Resourse) + ' ' + String(x.ResourseMeasure)
                    : ''
            } .
       ${x.Color ? 'Цвет - ' + x.Color : ''}
        </td>
        <td>${x.Amount}</td>
    </tr>`)
    );
